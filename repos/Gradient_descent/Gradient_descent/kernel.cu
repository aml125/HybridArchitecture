...
cudaError_t gradientDescentWithCuda(double* vars, double* res, int NPOINTS, int ITERATIONS, double* dev_vars)
{
    double* dev_res = 0;
    ...
    gradientKernel << <NPOINTS / 1000, 1000 >> > (dev_vars, dev_res, ITERATIONS);
...
int launchTest(int NPOINTS, int ITERATIONS, int processes) {
    ...
    {
        std::thread thread(gradientDescentWithCuda, vars, res, NPOINTS, ITERATIONS, dev_vars);
        thread.detach();
        /*cudaError_t cudaStatus = gradientDescentWithCuda(vars, res, NPOINTS, ITERATIONS, dev_vars);
...
int main()
{
    ...
    {
        tm.StartCounter();
        if (launchTest(NPOINTS, ITERATIONS, i) != 0) {
